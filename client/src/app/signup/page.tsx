...
const SignupPage: React.FC = () => {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    if (username.length > 0 && password.length > 0) {
      try{
          const res = await axios.post(`${BASE_URL}/createnew`, {
          username,
          password,
        });
        if(res.status === 201){
  ...
  return (
    <div className="flex md:flex-row flex-col items-center justify-between min-h-screen ">
      ...
      <div className="w-full h-full max-md:hidden">
        <Image
          src="https://plus.unsplash.com/premium_photo-1669926596082-7c457a85c901?q=80&w=2044&auto=format&fit=crop&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D"
          height={1000}
          width={1000}
          alt="hellow"
